...
function ElementPicker({ types, close, choose }) {
    const option = (item) => _jsx("button", { type: "button", onClick: () => choose(item), style: { padding: '10px 12px', textAlign: 'left', borderRadius: 6 }, children: item.label }, `${item.kind}:${item.value}`);
    return _jsx("div", { role: "presentation", onMouseDown: (event) => { if (event.currentTarget === event.target)
            close(); }, style: { position: 'fixed', inset: 0, zIndex: 1000, display: 'grid', placeItems: 'center', padding: 24, background: 'rgba(0,0,0,.32)' }, children: _jsxs("section", { role: "dialog", "aria-modal": "true", "aria-labelledby": "element-picker-title", style: { ...panel, width: 'min(680px, 100%)', maxHeight: 'min(720px, 85vh)', overflowY: 'auto', background: 'var(--theme-elevation-0)', boxShadow: '0 18px 60px rgba(0,0,0,.24)' }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }, children: [_jsxs("div", { children: [_jsx("h2", { id: "element-picker-title", style: { marginBottom: 4 }, children: "Add an element" }), _jsx("p", { style: { marginTop: 0 }, children: "Choose what to place in this area." })] }), _jsx("button", { type: "button", "aria-label": "Close element picker", onClick: close, style: { width: 32, height: 32, padding: 0 }, children: "\u00D7" })] }), _jsx("h3", { children: "Layout" }), _jsx("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8 }, children: layoutItems.map(option) }), _jsx("h3", { children: "Fields" }), _jsx("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8 }, children: fieldItems.map(option) }), !!types.length && _jsxs(_Fragment, { children: [_jsx("h3", { children: "Designed Blocks" }), _jsx("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8 }, children: types.map((type) => option({ kind: 'block', value: String(type.id), label: type.name })) })] })] }) });
}
