...
export function TemplateContentEditor({ lexicalSchemaPath }) {
    ...
    return _jsxs("section", { className: "template-content-editor", children: [_jsx("h3", { children: "Template content" }), template.sections.map((section) => _jsxs("fieldset", { className: "template-content-editor__section", children: [_jsxs("legend", { children: [section.name, section.required ? ' (required)' : ''] }), section.required && (sections[section.key]?.fields ?? []).some((field) => field.required && !values[section.key]?.[field.key]) &&
                        _jsx("p", { role: "alert", children: "This section is missing required content. Fill it before publishing this Post." }), section.allowDesignOverride && _jsxs("label", { className: "template-content-editor__field", children: ["Design", _jsxs("select", { disabled: disabled, value: String(overrides[section.key] ?? ''), onChange: (event) => setOverrides({ ...overrides, [section.key]: event.target.value || null }), children: [_jsx("option", { value: "", children: "Use Template Default" }), sections[section.key]?.designs.map((design) => _jsx("option", { value: String(design.id), children: design.name }, design.id))] })] }), (sections[section.key]?.fields ?? []).map((field) => _jsx("div", { className: "template-content-editor__field", children: _jsxs("label", { children: [field.label, field.required ? ' *' : '', field.kind === 'textarea' ? _jsx("textarea", { disabled: disabled, value: String(values[section.key]?.[field.key] ?? ''), onChange: (event) => update(section.key, field.key, event.target.value) })
                                    : field.kind === 'boolean' ? _jsx("input", { disabled: disabled, type: "checkbox", checked: Boolean(values[section.key]?.[field.key]), onChange: (event) => update(section.key, field.key, event.target.checked) })
                                        : field.kind === 'media' ? _jsxs("select", { disabled: disabled, value: String(values[section.key]?.[field.key] ?? ''), onChange: (event) => update(section.key, field.key, event.target.value || null), children: [_jsx("option", { value: "", children: "Choose Media" }), mediaOptions.map((media) => _jsx("option", { value: String(media.id), children: media.alt || media.filename || media.id }, media.id))] })
                                            : field.kind === 'group' ? _jsx("div", { children: field.children?.map((child) => _jsxs("label", { className: "template-content-editor__field", children: [child.label, _jsx("input", { disabled: disabled, value: String(values[section.key]?.[field.key]?.[child.key] ?? ''), onChange: (event) => update(section.key, field.key, { ...((values[section.key]?.[field.key]) ?? {}), [child.key]: event.target.value }) })] }, child.key)) })
                                                : _jsx("input", { disabled: disabled, type: field.kind === 'number' ? 'number' : field.kind === 'date' ? 'date' : 'text', value: String(values[section.key]?.[field.key] ?? ''), onChange: (event) => update(section.key, field.key, field.kind === 'number' ? Number(event.target.value) : event.target.value) })] }) }, field.key))] }, section.key)), (() => { const legacy = templateFields(template.layout ?? []).filter((field) => !field.content || field.content.source === 'custom'); const custom = template.customFields ?? []; const all = [...custom.map((field) => ({ ...field, type: 'field' })), ...legacy.filter((field) => !custom.some((item) => item.id === field.id))]; return all.length > 0 && _jsxs("div", { children: [_jsxs("h3", { children: [template.name, " fields"] }), all.map((field) => _jsx("div", { className: "template-content-editor__field", children: _jsx(DynamicTemplateField, { field: field, lexicalSchemaPath: lexicalSchemaPath, value: values[field.id], disabled: disabled, onChange: (value) => setValues({ ...values, [field.id]: value }) }) }, field.id))] }); })(), Object.keys(values).filter((key) => !template.sections.some((section) => section.key === key) && !templateFields(template.layout ?? []).some((field) => field.id === key) && !(template.customFields ?? []).some((field) => field.id === key)).length > 0 && _jsx("p", { children: "Values for removed fields remain stored for recovery." })] });
}
function DynamicTemplateField({ field, lexicalSchemaPath, value, disabled, onChange }) {
    const label = `${field.label}${field.required ? ' *' : ''}`;
    const { config } = useConfig();
    if (field.fieldType === 'toggle')
        return _jsxs("label", { className: "template-content-editor__toggle", children: [label, _jsx("input", { disabled: disabled, type: "checkbox", checked: Boolean(value), onChange: (event) => onChange(event.target.checked) })] });
    ...
    if (field.fieldType === 'select')
        return _jsxs("label", { children: [label, _jsxs("select", { disabled: disabled, "aria-label": label, value: String(value ?? ''), onChange: (event) => onChange(event.target.value), children: [_jsx("option", { value: "", children: "Choose an option" }), (field.options ?? []).map((option) => _jsx("option", { value: option, children: option }, option))] }), field.helpText && _jsx("small", { children: field.helpText })] });
