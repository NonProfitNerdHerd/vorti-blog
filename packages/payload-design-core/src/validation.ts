...
export function validateFieldDefinitions(fields: FieldDefinition[]): ValidationIssue[] {
  ...
  for (const field of fields) {
    if (!/^[a-z][a-zA-Z0-9]*$/.test(field.key)) issues.push({ path: field.key, message: 'Field key must use a stable camelCase identifier' })
    if (keys.has(field.key)) issues.push({ path: field.key, message: 'Duplicate field key' })
    keys.add(field.key)
    if (field.kind === 'group') {
      if (!field.children?.length) issues.push({ path: field.key, message: 'Group needs child fields' })
      else issues.push(...validateFieldDefinitions(field.children).map((issue) => ({ ...issue, path: `${field.key}.${issue.path}` })))
    } else if (field.children?.length) issues.push({ path: field.key, message: 'Only groups may have child fields' })
    if (field.kind === 'select' && !field.options?.length) issues.push({ path: field.key, message: 'Select needs options' })
  }
  return issues
}

export function validateContentValues(type: Pick<BlockType, 'fields'>, values: unknown, requiredSection = true): ValidationIssue[] {
  if (!isObject(values)) return [{ path: '', message: 'Content values must be an object' }]
  const issues: ValidationIssue[] = []
  function check(fields: FieldDefinition[], object: Record<string, unknown>, prefix: string) {
    const allowed = new Set(fields.map((field) => field.key))
    for (const key of Object.keys(object)) if (!allowed.has(key)) issues.push({ path: `${prefix}${key}`, message: 'Unknown content field' })
    for (const field of fields) {
      const value = object[field.key]
      const path = `${prefix}${field.key}`
      if (value == null || value === '') {
        if (requiredSection && field.required) issues.push({ path, message: `${field.label} is required` })
        continue
      }
      switch (field.kind) {
        case 'text': case 'textarea':
          ...
          break
        case 'url':
          if (typeof value !== 'string' || !/^https?:\/\/|^\//.test(value)) issues.push({ path, message: 'Must be an http(s) URL or local path' })
          break
        case 'number':
        ...
        case 'boolean':
          ...
          break
        case 'select':
          if (typeof value !== 'string' || !field.options?.includes(value)) issues.push({ path, message: 'Must be one of the supported options' })
          break
        case 'media':
          if (!(typeof value === 'number' || typeof value === 'string' || (isObject(value) && ('id' in value)))) issues.push({ path, message: 'Must reference a media item' })
          break
        case 'date':
          if (typeof value !== 'string' || Number.isNaN(Date.parse(value))) issues.push({ path, message: 'Must be a valid date' })
          break
        case 'group':
          if (!isObject(value)) issues.push({ path, message: 'Must be an object' })
          else check(field.children ?? [], value, `${path}.`)
          break
      }
    }
  }
