...
test.describe('Admin Panel', () => {
  ...
  test('builds a Newsletter with drag and drop, stable fields, Hero mappings, and nested columns', async () => {
    ...
    try {
      ...
      await expect(builder.getByRole('combobox', { name: 'Column Layout' })).toBeVisible({ timeout: 10_000 })
      ...
      const richTextHandle = await builder.getByRole('button', { name: 'Drag Rich Text' }).boundingBox()
      const leftDropZone = await left.boundingBox()
      expect(richTextHandle).not.toBeNull()
      expect(leftDropZone).not.toBeNull()
      await page.mouse.move(richTextHandle!.x + richTextHandle!.width / 2, richTextHandle!.y + richTextHandle!.height / 2)
      await page.mouse.down()
      await page.mouse.move(leftDropZone!.x + leftDropZone!.width / 2, leftDropZone!.y + leftDropZone!.height / 2, { steps: 12 })
      await page.mouse.up()
      await expect(builder.getByRole('textbox', { name: 'Field Label' })).toBeVisible({ timeout: 10_000 })
