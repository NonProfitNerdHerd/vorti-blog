...
test.describe('Admin Panel', () => {
  ...
  test('opens Standard Article with legacy references preserved in the visual builder', async () => {
    ...
    try {
      ...
      await expect(builder.getByRole('heading', { name: 'Block Library' })).toBeVisible()
  ...
  test('builds a Newsletter with drag and drop, stable fields, Hero mappings, and nested columns', async () => {
    ...
    try {
      ...
      await builder.getByRole('button', { name: 'Save Element' }).click()
      await expect(columnsCard).toContainText('Main Story')
      await expect(columnsCard).toContainText('Story Image')
      ...
      await expect(builder.getByRole('heading', { name: 'Block Library' })).toBeHidden()
