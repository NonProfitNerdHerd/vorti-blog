...
export function PostDesignSections({ sections }: { sections: PostDesignSection[] }) { return <>{sections.map((section) => {
  ...
  if (section.kind === 'field') { if (section.value == null || section.value === '') return null; if (section.fieldType === 'richText' && typeof section.value === 'object' && section.value && 'root' in section.value) return <div key={section.key} className="template-field template-field--richText"><RichText data={section.value as never} className="rich-text" /></div>; if (section.fieldType === 'image' && typeof section.value === 'object' && section.value && 'url' in section.value) return <img key={section.key} src={String((section.value as { url: unknown }).url)} alt={section.label} />; if (section.fieldType === 'images' && Array.isArray(section.value)) return <div key={section.key} className="template-gallery">{section.value.map((value, i) => typeof value === 'object' && value && 'url' in value ? <img key={i} src={String((value as { url: unknown }).url)} alt={String((value as { alt?: unknown }).alt ?? '')} /> : null)}</div>; return <div key={section.key} className={`template-field template-field--${section.fieldType}`}><h2>{section.label}</h2><p>{String(section.value)}</p></div> }
